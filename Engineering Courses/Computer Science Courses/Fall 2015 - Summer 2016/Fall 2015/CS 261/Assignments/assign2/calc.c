...
void natural(struct DynArr *stack)
{
	
}

void logr(struct DynArr *stack)
{
...
double calculate(int numInputTokens, char **inputString)
{
	...
	for(i=1;i < numInputTokens;i++) 
	{
		...
		else 
		{
			...
			//check = isNumber(s, num);
			//if(check == 1){
				/*if(stack->data[0] != NULL){
					stack[1] = s;
				}
				else{
					stack[0] = s;
			...
			if(strcmp(s, "pi") == 0){
				...
				printf("You inputted an invalid command line. Try again.\n");
				exit(0);
			}
