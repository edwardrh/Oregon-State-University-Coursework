...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    else if(compare(val, tree->root->val) == -1 && tree->root->left != NULL){ //left < right
		tree->root = tree->root->left;
		containsBSTree(tree, val);
	}
	else if(compare(val, tree->root->val) == 1 && tree->root->right != NULL){ //left > right
...
TYPE _leftMost(struct Node *cur)
{
	...
	else if(cur->left->left->left == NULL){
		return cur->left->left->val;
	}
	/*else{
		_leftMost(cur->left);
	}
	Recursive aspect is not meeting conditions prompted*/
    return cur->val;
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
    //FIXED
	if(cur->left == NULL){
		free(cur); //or remove this statement
		return cur->right; //or return NULL
	}
	else if(cur->left->left == NULL){
		free(cur->left);
		cur->left = 0;
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	/*if(compare(val, cur->val) == 0){
		free(cur);
		return cur; //I don't think this is valid
	}*/
    if(compare(val, cur->val) == -1){ //left < right
		if(cur->left->left == NULL){
			if(compare(val, cur->left) == 0){
				free(cur->left);
				return cur;
			}
			else{
	...
	else if(compare(val, cur->val) == 1){ //left > right
		if(cur->right->right == NULL){
			if(compare(val, cur->right) == 0){
				free(cur->right);
				return cur;
			}
			else{
