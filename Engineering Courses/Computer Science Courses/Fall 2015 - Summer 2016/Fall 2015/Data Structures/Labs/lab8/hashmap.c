...
void insertMap(struct hashMap *map, char *key, int value) {
	...
	int hashIndex = _hashString(key) % map->tableSize;
	struct hashLink * newHash = (struct hashLink *);
	...
	if(map->table(hashIndex) == NULL){
		map->table(hashIndex) = newHash;
		map->count++;
		return;
	}
	
	struct hashLink * p = (struct hashLink *) map->table(hashIndex);
