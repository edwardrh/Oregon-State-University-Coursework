...
Display( )
{
	...
	if (Textures != 0) {
		...
		if (Person == 0) {
			...
			MjbSphere(5., 50, 50);
			if (Move != 0) {
				glMatrixMode(GL_TEXTURE); //For transformations
...
InitMenus( )
{
	...
	int personmenu = glutCreateMenu(DoPersonMenu);
	glutAddMenuEntry("Phoebe", 0);
	glutAddMenuEntry("Melissa", 1);
	...
	glutAddSubMenu("Person", personmenu);
