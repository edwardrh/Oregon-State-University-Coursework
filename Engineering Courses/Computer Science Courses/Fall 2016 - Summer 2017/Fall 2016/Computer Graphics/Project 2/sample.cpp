...
#include "heli.550.txt"

// blade parameters:

#define BLADE_RADIUS 5.0
#define BLADE_WIDTH 2.0
#define BLADE_RADIUSB 1.5
#define BLADE_WIDTHB 0.6
...
Animate( )
{
	...
	int MS_IN_THE_ANIMATION_CYCLE = { 300 };
...
InitLists( )
{
	...
	// creating helicopter
		...
		glBegin(GL_LINES);
		glColor3f(0., 1., 1.);
		for (w = 0, ep = Heliedges; w < Helinedges; w++, ep++)
		{
			p0 = &Helipoints[ep->p0];
			p1 = &Helipoints[ep->p1];
	...
	// creating helicopter blades
		...
		glColor3f(1., 0., 0.);
		glVertex3f(BLADE_RADIUS, 2.9, BLADE_WIDTH / 2.);
		glVertex3f(0., 2.9, -2.);
		glVertex3f(BLADE_RADIUS, 2.9, -BLADE_WIDTH / 2.);
		glEnd();
		...
		BladeListb = glGenLists(1);
		glNewList(BladeListb, GL_COMPILE);

		glBegin(GL_TRIANGLES);
		glColor3f(1., 0., 0.);
		glVertex3f(.5, 2.5+BLADE_RADIUSB, 9+(BLADE_WIDTHB / 2.));
		glVertex3f(.5, 2.5, 9.);
		glVertex3f(.5, 2.5+BLADE_RADIUSB, 9-(BLADE_WIDTHB / 2.));
		glEnd();
	...
	// object in front of the helicopter

		TwoList = glGenLists(1);
		glNewList(TwoList, GL_COMPILE);
