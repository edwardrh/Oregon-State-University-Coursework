...
const int VEXMAX1 = { 105 };
const int COLORMAX = { 6 };

// Animations

const int MS_PER_CYCLE = { 20000 };

// animate helicopter global variables
...
int		Textures;				// != 0 means no textures
int		Mess;					// != 0 means no distortions
int		Person;					// != 0 means phoebe
int		Move;					// != 0 means distort is on
int		AxesOn;					// != 0 means to draw the axes
int		LinesOn;				// != 0 means to draw the control lines
int		PointsOn;				// != 0 means to draw the control points
int		DebugOn;				// != 0 means to print debugging info
int		DepthCueOn;				// != 0 means to use intensity depth cueing
GLuint	BoxList;				// object display list
GLuint	StatCurves;				// stationary curves display list
GLuint	AniCurves;				// animated curves display list
GLuint	StatPoints;				// stationary curves display list
GLuint	AniPoints;				// animated curves display list
GLuint	StatLines;				// stationary curves display list
GLuint	AniLines;				// animated curves display list
GLuint	HeliList;				// helicopter display list
GLuint	BladeList;				// top blade display list
GLuint	BladeListb;				// rear blade display list
GLuint	TwoList;				// project 2 front object display list
...
GLuint  tex0, tex1;				// for the two textures
...
Display( )
{
	...
	glCallList( StatCurves );
	if (LinesOn != 0) {
		glCallList( StatLines );
	}
	if (PointsOn != 0) {
		glCallList( StatPoints );
	}

	glRotatef(360.*Time, 0., 0.25, 0.);
	glCallList( AniCurves );
	if (LinesOn != 0) {
		glCallList( AniLines );
	}
	if (PointsOn != 0) {
		glCallList( AniPoints );
	}
...
InitMenus( )
{
	...
	glutAddSubMenu("Mess", messmenu);
	glutAddSubMenu("Control Points", pointsmenu);
	glutAddSubMenu("Control Lines", linesmenu);
...
InitGraphics( )
{
	...
	TextureArray0 = BmpToTexture("phoebe.bmp", &widtha, &heighta);
	TextureArray1 = BmpToTexture("melissa.bmp", &widthb, &heightb);
...
InitLists( )
{
	...
	StatCurves = glGenLists( 1 );
	glNewList(StatCurves, GL_COMPILE );
	...
	AniCurves = glGenLists(1);
	glNewList(AniCurves, GL_COMPILE);
	...
	StatPoints = glGenLists(1);
	glNewList(StatPoints, GL_COMPILE);
	...
	AniPoints = glGenLists(1);
	glNewList(AniPoints, GL_COMPILE);
	...
	glVertex3f(-1.5, 0., 0.);
	glColor3f(1.0, 1.0, 0.);
	glVertex3f(-1.75, -0.25, -0.25);
	glVertex3f(-0.75, -0.25, -1.25);
	glColor3f(0., 1.0, 0.);
	glVertex3f(-0.25, 0., -1.25);
	glVertex3f(-0.25, 0., -1.25);
	glColor3f(1.0, 1.0, 0.);
	glVertex3f(0., -0.25, -1.5);
	glVertex3f(1.5, -0.25, -0.5);
	glColor3f(0., 1.0, 0.);
	glVertex3f(1.25, 0., -0.25);
	glVertex3f(1.25, 0., -0.25);
	glColor3f(1.0, 1.0, 0.);
	glVertex3f(1.75, -0.25, -0.5);
	glVertex3f(1.5, -0.25, 0.5);
	glColor3f(0., 1.0, 0.);
	glVertex3f(1.25, 0., 0.25);
	glVertex3f(1.25, 0., 0.25);
	...
	StatLines = glGenLists(1);
	glNewList(StatLines, GL_COMPILE);
	...
	AniLines = glGenLists(1);
	glNewList(AniLines, GL_COMPILE);
